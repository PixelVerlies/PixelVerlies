...
class itembar():
    ...
    def drawItemBar(self, rod, SCREEN, blockSize, textKoerper):
        x = 33
        y = 8
        ab = 2
        
        bew = textFunctions.textField(f"Taste", x - 1, y - 2, textKoerper, rev=1)
        bew.drawField(blockSize, SCREEN)
        bew = textFunctions.textField(f"Anzahl", x - 1 + 3, y - 2, textKoerper, rev=1)
        bew.drawField(blockSize, SCREEN)
        ...
        for i in range(self.counter):
            ...
            bew = textFunctions.textField(f"{3}", x + 3, y + (ab * i) + 1, textKoerper, rev=1)
            bew.drawField(blockSize, SCREEN)
