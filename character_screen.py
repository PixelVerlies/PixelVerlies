...
def create_character_fields(ueberschrift, textKoerper, selected_character_id=None):
    ...
    error_message = textField("", 2, 19, textKoerper, 25)
    ...
    show_armor_table = False  # Neu: Steuert welche Tabelle angezeigt wird
    ...
    if character_details:
        ...
        texfield_list.append(textField(f"Schutz: {character_details[8]}", 2, 15, textKoerper, 10))
        ######!!! schutz aus sql machen.
        ...
        if character_potions:
            for i, (desc, count) in enumerate(character_potions):
                texfield_list.append(textField(f"{count}x {desc}", 13, potion_y_start + (i*2), textKoerper, 9))
        else:
            texfield_list.append(textField("Keine Heiltränke", 13, potion_y_start, textKoerper, 9))
        
        texfield_list.append(textField(f"Waffe: {character_details[5] if character_details[5] is not None else 'Keine Waffe'}", 13, 11, textKoerper, 9))

        texfield_list.append(textField(f"Rustung:{character_details[7]}", 13, 13, textKoerper, 9))

        tabelle_button = toggleButton("Rustungstabelle", 13, 15, textKoerper, 9, 1)  
    ...
    weapon_table_headers = [["Name", "Schaden", "Gerustet"]]
    ...
    return {
        ...
        "armor_table_headers": [["Name", "Schutz", "Gerustet"]],
...
def draw_armor_inventory_table(character_data, blockSize, SCREEN, textKoerper):
    full_armor_data = character_data["armor_table_headers"] + character_data["armor_table_data"]
    
    table_font = pygame.font.Font("Arcade-Classic-Font/bytebounce.medium.TTF", 20)

    character_data["armor_table_rects"] = []
    table_start_x = 23
    table_start_y = 5
    col_widths = [8, 3, 4]  # Breite für Name, Schutz, Gerüstet

    for row_idx, row_data in enumerate(full_armor_data):
    ...
    if event.type == pygame.MOUSEBUTTONDOWN:
        ...
        if character_data.get("fertig_button") and character_data["fertig_button"].rec.collidepoint(posx, posy):
            if character_data["char_name_input"] and character_data["selected_character_id"]:
                ...
                current_name = sql.get_character_details(character_data["selected_character_id"])[0]
